		...
		for _, stream := range camera.Streams {
			go func(camera Camera, stream StreamC) {
				for {
					...
					cmdStr := fmt.Sprintf("/usr/local/bin/ffmpeg -hide_banner %s-fflags nobuffer -rtsp_transport %s -i  %s -c:v copy %s -async 1 -movflags empty_moov+omit_tfhd_offset+frag_keyframe+default_base_moof -frag_size 10 -f mp4 %s", stimeout, rtspTransport, uri, audio, stream.MediaServerInputUri)
