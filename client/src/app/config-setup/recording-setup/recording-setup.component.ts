...
export class RecordingSetupComponent implements OnInit, AfterViewInit {
  ...
  camera: InputSignal<Camera | undefined | null> = input<Camera | undefined | null>();
  ...
  public cameraSvc = inject(CameraService);
  private parent = inject(ConfigSetupComponent);
  ...
  confirmChanges() {
    if (this.camera() !== undefined && this.camera() !== null) {
      ...
      camera.recordingType = cam.recordingType;
      camera.motion_detection_stream = cam.motion_detection_stream;
      camera.recordingStream = cam.recordingStream;
      camera.retriggerWindow = cam.retriggerWindow;
      camera.pullPointTopic = cam.pullPointTopic;
      camera.simpleItemName = cam.simpleItemName;
      camera.simpleItemPositiveValue = cam.simpleItemPositiveValue;
  ...
  setupData(): void {
    if (this.camera())
      this.localCamera = this.clone(this.camera() as Camera);
    this.setUpFormGroup();
  }
