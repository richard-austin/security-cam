...
start () {
	ffmpeg-v4.4.4 -i "$RECORDING_SRC_URL" -t 01:00:00 -an -c copy -level 3.0 -start_number 0 -hls_time 3 -hls_list_size 0 -hls_segment_type fmp4 -hls_fmp4_init_filename "$LOCATION-$(date "+%s")_.mp4" -f hls /var/security-cam/"$LOCATION/$LOCATION-$(date "+%s")_.m3u8" &
