...
start () {
	...
	echo "$(date +%d-%m-%Y" "%T): Started recording- ffmpeg -re -i rtmp://localhost:1935/${FIELDS[1]}/${FIELDS[2]}" -an -c copy -level 3.0 -start_number 0 -hls_time 3 -hls_list_size 0 -f hls /home/security-cam/"${FIELDS[3]}/${FIELDS[1]}-$(date "+%s")_.m3u8" >> "${log_file}"

  # Remove oldest recording files (older than 2 weeks)
  find /home/security-cam/"${FIELDS[3]}" -mmin +20160 -exec rm {} \;
}
